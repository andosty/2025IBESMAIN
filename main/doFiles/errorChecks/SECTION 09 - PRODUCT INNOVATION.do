...
decode s9_q1a, generate(s9_q1a_string)
decode s9_q1b, generate(s9_q1b_string)
decode s9_q1c, generate(s9_q1c_string)
...
replace errorMessage = "total innovations% ='" + string(totalInnov_andOther_Percent) + "', is invalid, for a firm with product innovation . Section9 Q1a  (New to Firm), b (New to Gh Mkt) and c (New to rest of World) answeres were " + s9_q1a_string+ ", " + s9_q1b_string + " , " + s9_q1c_string if $innovativeManufacturer & $invalidPercInnovManufacturer
...
replace errorMessage = "the described main product innovation ='" + s9q8 + "' is invalid" if $badInnovationDescription
...
replace errorMessage = varNameNew +  " InnovationObjective cannot be blank , if Q91a,b,c are " + s9_q1a_string+ ", " + s9_q1b_string + " , " + s9_q1c_string if $missingInnovationObjective
...
// --------------------------------------------------------------------------------------------
use "$prepData\ibes_ii Estabs valid_dateCase_only.dta", clear
gl otherSpec_InnovObj_missing ($innovativeManufacturer & s9q17b== 1 & (missing(s9q17o) | wordcount(s9q17o) < 2 | strlen(s9q17o) < 4 )) 
drop s8q1__0-s8q4c  // drop section 8 revenue vars
ren (s9q1a s9q2 s9q3 s9q8) (s9_q1a s9_q1b s9_q1c s9q8 ) // rename to match the alphabet que numbering
...
* check for missing, invalid response and out of range
		...
		replace errorMessage = "Answer to Q4.H Other Specify is 'Yes', but failed to provided the detailed specification."  if $otherSpec_InnovObj_missing & missing(s9q17o)
		
	replace errorMessage = "Answer to Q4.H Other Specify is 'Yes', but the provided other specification ='" + s9q17o +"' is invalid "  if $otherSpec_InnovObj_missing & (wordcount(s9q17o) < 2 | strlen(s9q17o) < 4 )
...
gl inconsistent_InnovObj ($innovativeManufacturer & /// 
 (inlist(s9_q1c== 2,3) & s9q12==1) & /// no export innovation but has export innov objectives
 ((inlist(s9_q1a== 2,3) | inlist(s9_q1b== 2,3)) &  s9q11==1) ) // no domestic innovation but has domestic innov objectives
...
// check for missing, invalid response and out of range
		...
		replace errorMessage = "Q14.H objective to export to foreign market =' "+ string(s9q12)  +"'. But Q1.c. if firm made any innovation to the rest of the world = '"+ string(s9_q1c) + "'" if $inconsistent_InnovObj & (inlist(s9_q1c== 2,3) & s9q12==1)
		
		replace errorMessage = "Q14.C objective to increase domestic market =' "+ string(s9q11)  +"'. But Q1.a If innovations New to firm ='"+ string(s9_q1a)+"' and Q1b If innovations New to Ghanaian Market ='" + string(s9_q1b) + "'" if $inconsistent_InnovObj & ((inlist(s9_q1a== 2,3) | inlist(s9_q1b== 2,3)) &  s9q11==1)  
